const contentLv = {
  nav1: 'Menu',
  about: 'Par mums',
  brunch: 'Brančš',
  banquet: 'Banketi',
  contacts: 'Kontakti',
  flag1: 'Russian',
  flag2: 'English',
  hero1: 'Sveicināti',
  hero2: 'House of light',
  res_button: 'Veikt rezervāciju',
  about_title:
    'Laipni lūgti restorānā House of Light – mājīgā grilbārā pašā Jūrmalas sirdī!',
  about1:
    'Jūrmalas restorāns House of Light ir vieta, kur atpūsties no ikdienas steigas, baudīt gardu maltīti vai atspirdzinošu dzērienu',
  about2:
    'Restorāna plašajā ēdienkartē esam iekļāvuši svaigus salātus, uzkodas, uzkodu plates, izcilus jūras velšu ēdienus, veģetārus un vegānus ēdienus, pašu kūpinātu zivis un gaļu, burgerus, pastas, ēdienus gatavotus uz grila, kā arī izcilus desertus. Restorāna House of Light piedāvājumā ir arī Latvijā ražoti vīni un Latvijā grauzdēta kafija.',
  about3:
    'Mīļi gaidām arī ģimenes ar bērniem, jo restorāna telpās esam parūpējušies par atsevišķu krāsojamo bērnu ēdienkarti, atraktīvu bērnu stūrīti, āra bērna laukumiņu, kā arī pārtinamo galdiņu pašiem mazākajiem.',
  brunch_title: 'Svētdienas brančs',
  brunch_about:
    'Lai arī mūsu ierastos Brančus vēl nodrošināt nevaram, ņemot vērā lielo pieprasījumu pēc brīvdienu vēlajām brokastīm, esam izveidojuši jaunu konceptu – “Mini Branči”, kas ļauj mums parūpēties par jūsu drošību, uzklājot Mini Branča galdu tieši Jums!',
  brunch_list: [
    'Katru svētdienu, 12:00 – 15:00',
    'Grupām sākot no 6 cilvēkiem',
    'Katru nedēļu jauns piedāvājums',
    '20 € no personas',
    'Bērniem līdz 6 gadu vecumam par brīvu, no 7 gadiem - 10€',
    'Cenā iekļauta glāze Prosecco vai karstvīns, ūdens, morss, kafija un tēja',
  ],
  banquet_title: 'Banketu piedāvājums',
  banquet_subtitle: 'Jūs svinat – mēs gatavojam!',
  banquet1:
    'Restorānā House of Light var pasūtīt visu – sākot no gardām uzkodu platēm, svaigiem salātiem un visdažādākajiem karstajiem ēdieniem, līdz pat mājas limonādēm un citiem dzērieniem.',
  banquet2: 'Nodrošinām transportu ēdiena piegādei',
  banquet3: 'Piedāvājam apkalpojošo personālu Jūsu pasākumam',
  banquet4: 'Vairāk informācijai rakstīt',
  banquet_email: 'raitis@hol.lv',
  reserve_title: 'Rezervēt',
  reserve_name: 'Vārds',
  reserve_name_placeholder: 'Kārlis',
  reserve_date: 'Datums',
  reserve_time: 'Laiks',
  reserve_count: 'Viesu skaits',
  reserve_phone: 'Telefona numurs',
  reserve_comments: 'Komentāri',
  reserve_comments_placeholder: 'Nepieciešams bērnu krēsliņš utt.',
  working_hours: 'Darba laiks',
  work_days: 'P. - Sv.',
  rights:
    'SIA House of Light, Visas tiesības aizsargātas. Restorāns House of Light Jūrmala.',
}

export default contentLv
